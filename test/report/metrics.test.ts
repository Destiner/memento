...
describe('computeMetrics (sample event logs)', () => {
    ...
    expect(metrics.callsByDay).toEqual([
      {
        date: '2026-07-01',
        total: 6,
        byTool: { create_memory: 2, search_memory: 2, read_memory: 2 },
      },
      {
        date: '2026-07-02',
        total: 5,
  ...
  it('accumulates memories created over time and by type/scope', () => {
    ...
    expect(metrics.memoriesByType).toEqual({ decision: 2, pattern: 1 });
    expect(metrics.memoriesByScope).toEqual({ project: 2, personal: 1 });
  });
  ...
  it('derives search health and cross-tool ratios', () => {
    ...
    // reads (3) / (creates 3 + updates 1)
    expect(metrics.readToWriteRatio).toBe(0.75);
  });

  it('computes latency percentiles per tool', () => {
    expect(metrics.latencyByTool.create_memory).toEqual({ count: 3, p50: 12, p90: 20, p99: 20 });
    expect(metrics.latencyByTool.search_memory).toEqual({ count: 3, p50: 40, p90: 50, p99: 50 });
    expect(metrics.latencyByTool.read_memory).toEqual({ count: 3, p50: 5, p90: 6, p99: 6 });
